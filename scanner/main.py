...
def get_investor_analysis(code):
    """최근 거래일 기준 수급 분석 (호출 방식 전면 수정)"""
    try:
        # 종료일 오늘, 시작일 10일 전
        end_date = datetime.now().strftime("%Y%m%d")
        start_date = (datetime.now() - timedelta(days=10)).strftime("%Y%m%d")
        
        # [수정] stock.get_... 방식이 에러가 나면 직접 stock 모듈의 함수를 타겟팅
        # 만약 이 이름도 안된다면 pykrx 버전이 아주 낮은 것이므로 .yml에서 업데이트가 필수입니다.
        df = stock.get_market_net_purchase_of_equities_by_ticker(start_date, end_date, code)

        if df is not None and not df.empty:
            # 해당 기간의 마지막 날(최근 거래일) 데이터 추출
...
def get_detailed_analysis(code):
    try:
        ...
        return {
            "price": current_price, "position_pct": position_pct, "rsi": rsi,
            "disparity": disparity, "buy_target": buy_target, "sell_target": sell_target,
            "stop_loss": stop_loss, "frgn_buy": f_buy, "inst_buy": i_buy, "is_double_buy": d_buy
        }
    except Exception as e:
...
def scan_stocks():
    ...
    for index, row in target_stocks.iterrows():
        ...
        if analysis:
            stock_data = {
                "name": name, "code": code, "price": analysis['price'],
                "position_pct": analysis['position_pct'], "rsi": analysis['rsi'],
                "disparity": analysis['disparity'], "buy_target": analysis['buy_target'],
                "sell_target": analysis['sell_target'], "stop_loss": analysis['stop_loss'],
                "frgn_buy": analysis['frgn_buy'], "inst_buy": analysis['inst_buy'],
                "is_double_buy": analysis['is_double_buy'], "updatedAt": datetime.now()
            }
            collection.insert_one(stock_data)
            print(f"✅ {name}: {analysis['frgn_buy']}/{analysis['inst_buy']} 반영")
        
        time.sleep(0.2)
    print("🎯 모든 데이터가 정상적으로 저장되었습니다.")
